...
Since v1.2.0 the till renders its own version, at the foot of `/pos/settings`
and of every App admin pane, so the number needs a constant:
`CASPIAN_POS_VERSION` in [../src/pos/standalone/pos-version.ts](../src/pos/standalone/pos-version.ts).
That file is **generated by `tsup.config.ts`** from this `package.json`, the same
way `src/version.ts` is generated from the root one, and is committed so
`tsc --noEmit` needs no prior build. Do not hand-edit it, and do not export it
from a barrel — nothing outside `src/pos/` has any use for it. Until v1.2.0
there was deliberately no such constant, on the grounds that generating a string
no screen displays is bookkeeping pretending to be code; a screen displays it
now, so the reasoning no longer holds.
...
`tsup.config.ts` is **not** on that table and is not on the till's own list
either. It writes `pos-version.ts`, so a change to how that file is generated is
a library change — but bumping this folder's `package.json` is not, because the
generator reads the number rather than carrying it.
