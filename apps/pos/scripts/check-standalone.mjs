...
// not the library's. It used to be the library's `dist/index.mjs`, back when
// the register shipped inside that package.
const entry = join(root, '.check', 'check-entry.js');
if (!existsSync(entry)) {
  console.error('[check-standalone] .check/check-entry.js is missing. Run `npm run build:check` first.');
  process.exit(1);
}
