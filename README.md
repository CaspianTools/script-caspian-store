...
```bash
npm install github:CaspianTools/script-caspian-store#v13.5.0 firebase
```
...
| **Point of sale:** `/pos` full-screen register — barcode scanning (USB/Bluetooth keyboard-wedge, camera, manual), cash / card / split tender with change due, 80 mm receipt, server-priced sales via the `caspian-pos` codebase; per-device language, appearance and scanner settings at `/pos/settings`; a collapsible side menu that folds to an icon rail and to a drawer on a tablet, plus a **dark mode** that can follow the operating system; register-only mode disables the storefront; **installs as its own PWA** (own icon and window, separate from the storefront app) via `buildPosWebManifest()`. Owner + cashier documentation in [`docs/pos-manual.html`](docs/pos-manual.html) | ✅ (v10.0.0+) |
| **Standalone till (no shop, no website, no Firebase):** mount the provider with `standalone` and the register runs entirely on one computer — catalogue, staff, sales and receipt numbers in IndexedDB, contacting nothing. Local sign-in against roles built from twelve fine-grained capabilities (seven ship built in, from Cashier to Support, and a shop can invent its own), a screen each for the catalogue (`/pos/store`), the takings (`/pos/sales`) and the staff (`/pos/people`), shop-and-receipt wording plus backup-and-restore as sections of `/pos/settings`, and the role editor at `/pos/app-admin`, which also carries an off-by-default switch that makes a cashier declare the cash in the drawer before the sale screen will open and keeps what each of them counted. **Store management** (v13.3.0+): every item has its own page at `/pos/store/<id>` with its description, its stock per size and a dated ledger of everything received, sold, returned and adjusted; **Receive stock** at `/pos/store/receive` takes a delivery in by scanner or by hand — creating items it does not recognise from the scan itself — and records unit cost, so margin and stock value are real figures; optional **batch tracking with expiry dates** sells earliest-date-first; and **Categories** and **Suppliers** are two more optional screens, all three switched on per shop from `/pos/app-admin`. Nothing syncs to a website; backups are files the shop keeps. See [Standalone till](INSTALL.md#13-standalone-till-no-website-no-firebase) | ✅ (v11.0.0+) |
