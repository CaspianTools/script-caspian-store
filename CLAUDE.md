...
Rules, indexes, and `collections.ts` move together. `pendingCheckouts` and `stripeEvents` are the one exception: server-only (written by the Stripe Functions via the Admin SDK, rules deny all client access) and deliberately without a ref in `collections.ts`.

**Composite indexes are a consumer deploy, so services avoid needing them.** `where(isActive == true)` + `orderBy(...)`, or a status equality + `orderBy(createdAt)`, each require a composite index that a fresh project does not have; the query then throws `failed-precondition`, and every caller that swallowed that error rendered an empty list (v15.0 shipped with no categories on the storefront for exactly this reason). Filter or sort small lists in memory (`listActiveBrands`, `listActiveCategories`, `listAllReviews` are the pattern) and reserve composite indexes for large collections where the index is already shipped in `firestore.indexes.json`.
